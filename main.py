...
async def send_course_content(chat_id, content, file_type, caption):
    try:
        ...
        await bot.send_message(chat_id, f"❌ Kursni yuborishda muammo bo'ldi.")
...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    if not await check_sub(uid):
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            return await message.answer("⚠️ Kanallarga a'zo bo'ling:", reply_markup=kb.adjust(1).as_markup())

    await message.answer(f"🚀 Xush kelibsiz!", reply_markup=main_menu_kb(user['is_vip']))
...
async def view_courses(call: types.CallbackQuery):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if not courses: return await call.answer("Kurslar yo'q.", show_alert=True)
        kb = InlineKeyboardBuilder()
        for c in courses:
            kb.button(text=f"{c['title']} ({c['price']} ball)", callback_data=f"info_{c['id']}")
        kb.button(text="🔙 Bosh menyu", callback_data="back_home")
        await call.message.edit_text("📚 Kurslar:", reply_markup=kb.adjust(1).as_markup())
...
async def course_info(call: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await call.message.edit_text(f"📖 {course['title']}\n💰 Narx: {price}", reply_markup=kb.adjust(1).as_markup())
...
async def adm_c_add(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Nomini kiriting:")
...
async def adm_c_p(message: types.Message, state: FSMContext):
    ...
    await message.answer("Kontent yuboring (Fayl, Video yoki Link):")
